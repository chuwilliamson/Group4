	...
	SubShader 
	{
		Pass
		{
			...
			float4 frag(vertexOutput i) : COLOR
			{
				...
				localCoords.z = _BumpDepth;// 1(;) - 0.5 * dot(localCoords, localCoords);
				...
				// special stuff
				//_Shininess -= abs(sin(unity_DeltaTime * 3));
				//_Shininess = 5;
				...
				float3 rimLighting = saturate(dot(normalDirection, lightDirection) * _RimColor.xyz *
									 _LightColor0.xyz * pow(rim, _RimPower));
